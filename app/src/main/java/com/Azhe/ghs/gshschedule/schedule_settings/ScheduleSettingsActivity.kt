package com.Azhe.ghs.gshschedule.schedule_settings
...
class ScheduleSettingsActivity : BaseListActivity(), ColorPickerFragment.ColorPickerDialogListener {
    ...
    private var appearanceOnly = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.table = intent.extras?.getParcelable<TableBean>("tableData")
            ?: TableBean(id = 0, tableName = "")
        appearanceOnly = intent.extras?.getBoolean("appearanceOnly", false) ?: false
    ...
    private fun onItemsCreated(items: MutableList<BaseSettingItem>) {
        if (appearanceOnly) {
            // 外观模式
            items.add(CategoryItem("课表外观", true))
